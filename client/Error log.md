...
# I had an error with window.location.reload.
- I had been using this code to reload the page so that it reflects new information. 
- I was having trouble with tests because the tests are guiding me towards best practices and my code, while working, is a bit sloppy as I am now finding out.
- I think I've been using window.location.reload because it produces the results I wanted but I've found out that it's not really supposed to be used for single page application. I'm still not sure why. 
- Anyways, I have been using useHistory hook as an alternative. However, I encountered a problem because useHistory can't reload. It works fine if you're pushing to a different url than the current one but if you're just going to the same page
- These are problems I've had with other apps I've made to, just that writing the tests brought the issue to the fore.
- So, what I wanted to happen was for the user to see the updated list of images. Window.location.reload() did that. However, it was creating issues in tests.
- I used a modal for the add new image form and it automatically closes because the reload happens
- however, in tests, I guess there is no implementation of reload? I'm not really sure. In any case, the document structure still shows the modal instead of the images.
- My check was the alt text for the image would show up and it didn't because the document structure still showed the modal. The page didn't reload.
- I looked around the internet and one of the solution was to make sure the form was removed.
- I wrote that code and the test failed because the form remained.
- In any case, I spent a lot of time on this already so I prepared for sleep. 
- I don't know how but I guess what I've read for the past few hours probably nudged me to the solution I have now, which as I'm thinking about it, seems so simple I don't know why I didn't write it. I'm just glad the tests directed me to this solution. 
- What I did was close the form and make another fetch call to the images endpoint. There, I received the updated list of images. The images page component wouldn't update itself because I had the initial fetch call have no dependency so it only fetched the resources once. With add new image form, I fetched the updated list of images and set the images state to the updated one, making the images page rerender the list of images.
- So, yay
- So I guess in the future I will avoid window.location.reload? I'm still not sure why. I just know it produces less than desirable effects.
- I think one of the problems I encountered in this error was that the app was doing what it was supposed to do. The modal closes by itself and the new image appears. However, when I wrote what to expect in the test, it was failing despite the app actually working. I didn't really know where to start to debug. Was it the way I wrote the tests? I looked over how I implemented the msw thing. I did have some errors there too. I set up msw for the first version of annotate, the one with just the CRUD application so I didn't account that I later changed what I was sending to the server to an image container instead of two loose properties. Anyways, the point was that I was using the app and I was seeing that it was working. However, when I wrote the tests, the test was telling me the app wasn't working. So, that was pretty hard. Tracking down the error was hard. What I did was look at where things could possibly be going wrong. Was it how I wrote tests? Am I making a mistake in what queries I was using, the regex? Was it how I set up the mock server? I even changed my implementation to a surefire working way to see that I was writing the tests correctly. In the end, it was my implementation of the app that was the problem.

# Writing tests is directing me to best practices, I think
- For aesthetic reasons, I chose not to include the heading. I didn't think a label was really needed. The user can deduce as much that this is the iamges page. 
- However, when it came time to write tests and finding ways to indicate that I was on the right page, I caved to adding a heading because it was a pain doing without the heading.
- In the end, I think this was the right call. Not only because it made writing tests easier. I think it's also important for the app as a document. It might be weird to just have a list of images without really introducing what they are.
- So I included a heading as I was slightly coerced by the tests to do.
